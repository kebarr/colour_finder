...
def get_image_for_analysis(filename, region_y, region_x, len_y=24, len_x=10.5):
    os = openslide.OpenSlide(filename)
...
def get_blurred_image(image, kernel=kernel):
...
def remove_edges(results_array):
...
def find_damaged_region(image_array):
...
image = get_image_for_analysis(filename, (6.7, 9.9), (1.5, 4.5))
